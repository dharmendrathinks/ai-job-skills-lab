...
def page(kind, cards, metrics, filters, generated, scope, total, evidence='', tabs=None):
    title = 'Jobs evidence' if kind == 'jobs' else 'Projects & YouTube experiments'
    subtitle = ('Explore the captured engineering requirements behind your research. Availability is recorded at capture, not verified live.'
                if kind == 'jobs' else 'Useful projects, contributions and engineering stories. Explore the build, trade-offs and experiment behind each evidence-linked draft.')
    ...
    nav = ''.join('<a href="' + k + '.html"' + (' aria-current="page"' if k == kind else '') + '>' + t + '</a>'
                  for k,t in [('jobs','Jobs list'),('projects','Projects & YouTube')])
...
def render(state, now, limit):
    ...
    for group in openings[:limit]:
        ...
        for observation, captured in rows:
            card += '<details><summary>Description · ' + e(captured['source']) + '</summary><p class="prose">' + e(captured['description'] or 'Description not captured. Explicit requirements remain unknown.') + '</p><p>Source URL: ' + e(captured['url']) + '</p><p>Revision: ' + e(captured['source_revision']) + '</p><code>' + e(observation) + '</code></details>'
    ...
    for key,brief in selected:
        ...
        card='<article class="card '+kind+'" data-card data-kind="'+kind+'" '+attrs({'disposition':[disposition],'decision':[decision],'capability':caps})+'><div class="eyebrow">'+('PROJECT BRIEF' if kind == 'project' else 'YOUTUBE EXPERIMENT')+' / REVISION '+e(brief.get('revision',1))+'</div><h2>'+e(proposal.get('title','Project draft' if kind == 'project' else 'YouTube experiment draft'))+'</h2><div class="chips">'+chip(disposition,'good' if disposition in ('propose','contribute') else 'warn')+chip(decision,'blue')+''.join(chip(c,'purple') for c in caps)+'</div>'
        if proposal:
    ...
    projects_html=page('projects',rendered['project']+rendered['youtube'],
        [('Project briefs',len(by_kind['project'])),('YouTube experiments',len(by_kind['youtube'])),
         ('Insufficient evidence',sum(b.get('proposal',{}).get('disposition')=='insufficient-evidence' for _,b in current)),
         ('Pending review',sum(k not in decisions for k,_ in current))],
        ['disposition','decision','capability'],now,
        'Model-generated drafts, not completed work or observed video results. Job evidence supports relevance; audience demand needs separate validation.',
        len(current), tabs=tabs)
