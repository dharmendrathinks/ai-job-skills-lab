...
**Private reporting contact: awaiting maintainer confirmation.** A public release
must wait until this section contains an approved, reachable private address or
reporting URL. The private repository's vulnerability-reporting API returned 404
on 2026-09-11; availability of the GitHub form has not been established.

Existing collaborators may use their established private channel with Dharmendra
Thinks. Anyone without one should request contact details in a sanitized issue
without including vulnerability details, affected people, credentials or source
captures. Do not send fork-specific reports to the upstream maintainer. The issue
tracker is not a secret store, even while the repository is private.
...
The existing v0.1.0 prerelease predates the rename and connected learning workflow.
v0.2.0 is unreleased; fixes target current `main`. No older release or long-term
support line is promised. An
