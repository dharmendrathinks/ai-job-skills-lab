...
**Before using these instructions:** personal `/setup` and `/expand` writes are
blocked in this development template even while origin is private. Read
[application mode](docs/application-mode.md) for the separate private-workspace gate.
The upstream fork/install examples below do not authorize changing this repository’s
visibility or setting up personal data here.

Step-by-step instructions for the retained AI Job Search application framework.
